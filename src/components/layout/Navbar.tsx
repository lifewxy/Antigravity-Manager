...
function Navbar() {
    ...
    const toggleLanguage = async () => {
        if (!config) return;
        const langs = ['zh', 'zh-TW', 'en', 'ja', 'tr', 'vi', 'ru'] as const;
                ...
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="w-10 h-10 rounded-full bg-gray-100 dark:bg-base-200 hover:bg-gray-200 dark:hover:bg-base-100 flex items-center justify-center transition-colors"
                            title={t('nav.switch_to_' + (config?.language === 'zh' ? 'traditional_chinese' : config?.language === 'zh-TW' ? 'english' : config?.language === 'en' ? 'japanese' : config?.language === 'ja' ? 'turkish' : config?.language === 'tr' ? 'vietnamese' : config?.language === 'vi' ? 'russian' : 'chinese'))}
                        >
                            <span className="text-sm font-bold text-gray-700 dark:text-gray-300">
                                {t('nav.switch_to_' + (config?.language === 'zh' ? 'traditional_chinese_short' : config?.language === 'zh-TW' ? 'english_short' : config?.language === 'en' ? 'japanese_short' : config?.language === 'ja' ? 'turkish_short' : config?.language === 'tr' ? 'vietnamese_short' : config?.language === 'vi' ? 'russian_short' : 'chinese_short'))}
                            </span>
                        </button>
                    </div>
                </div>
